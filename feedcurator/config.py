"""Configuration constants and settings loader for feedcurator."""
...
class FetchSettings(BaseModel):
    ...
    timeout: int = 30
    user_agent: str = "feedcurator/0.1 (RSS curation pipeline)"
...
class NotifySettings(BaseModel):
    ...
    smtp_port: int = 465
    subject_template: str = "feedcurator — {date} ({count} new items)"
...
class PathSettings(BaseModel):
    ...
    templates_dir: Path = BASE_DIR / "feedcurator" / "templates"


class Settings(BaseSettings):
    """All tunable settings for feedcurator, loaded from environment variables and .env."""
